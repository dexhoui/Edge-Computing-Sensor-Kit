...
class RecoModelProcess(Process):
    ...
    def run(self):
        on_interpreter = tflite.Interpreter(model_path="../tflite_model/on.tflite")
        ...
        # 0: 97/96, 1: 58, 2:68, 3:78, 4:88, 5:94/93
        ...
        while True:
            ...
            if not self.queue_acc_fea.empty():
                ...
                on_output = on_interpreter.get_tensor(97)
                if on_output > 0.5:
                ...
                spin_output_a = spin_interpreter.get_tensor(88)
                spin_output_b = spin_interpreter.get_tensor(78)
                # print(spin_output, spin_output_a, spin_output_b)
                spin_output = np.mean((spin_output_b, spin_output_a, spin_output, spin_output))
                if spin_output > 0.5:
                ...
                bottom_output = bottom_interpreter.get_tensor(97)
                if bottom_output > 0.5:
                    pred_resutl.append('bottom')
                    #print(f"bottom : Y")
                else:
                    pred_resutl.append('')
                    #print(f"bottom : N")
                ...
                left_output = left_interpreter.get_tensor(93)
                if left_output > 0.5:
                    pred_resutl.append('left')
                    #print(f"left   : Y")
                else:
                    pred_resutl.append('')
                    #print(f"left   : N")
                ...
                right_output = right_interpreter.get_tensor(93)
                if right_output > 0.5:
                    pred_resutl.append('right')
                    #print(f"right  : Y")
                else:
                    pred_resutl.append('')
                    #print(f"right  : N")
