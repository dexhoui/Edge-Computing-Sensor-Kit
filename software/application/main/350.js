...
socket_client.onopen = function () {
    ...
    for (let i=0; i<72; i++){
        for (let j=0; j < 8; j++) {
            eyeOption.series[0].data.push([i, j, j*4]);
        }
    }
...
socket_client.onmessage = function (message) {
    ...
    if (distance > 10 && distance < 30) {
        spin.position.y = (distance - 16.7)/4.6 - 1
        console.log(distance, spin.position.y)
    }
...
bluePoint.position.set(-3, -5, 100);
scene.add(bluePoint);
...
const loader = new OBJLoader().setPath('model/350/');
...
mtlLoader.load('model/350/350.mtl', function(materials) {
    materials.preload();
    loader.setMaterials(materials);
    loader.load('350.obj', function(object) {
        object.scale.set(0.7, 0.7, 0.7);
        object.position.set(0, -5, 0);
        scene.add(object);
        renderer.render(scene, camera);
        hand = object.children[0];
        spin = object.children[1];
        console.log(hand)
    });
});
...
function animate() {
    ...
    renderer.render(scene, camera);
    if (spin.position.y < -1){
        spin.position.y = -1
    } else if (spin.position.y > 0){
        spin.position.y = 0
    }
